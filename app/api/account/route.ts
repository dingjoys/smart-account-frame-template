...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  let verified = data?.data?.credentialSymbol?.[data?.data?.credentialSymbol.length - 1] != "0"
  if (status == 1) {
    ...
    if (verified) {
      ...
      return new NextResponse(
        getFrameHtmlResponse({
          ...
          image: `${NEXT_PUBLIC_URL}/api/og?fid=${message.interactor.fid}&safe=${data.data.safeAddress}&balance=${encodeURIComponent(data.data.balance)}`,
